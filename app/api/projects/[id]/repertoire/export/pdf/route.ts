...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const dateStr = `${pad(nowDate.getDate())}${pad(nowDate.getMonth() + 1)}${nowDate.getFullYear().toString().slice(-2)}`;
    return new NextResponse(stream as any, {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="${project.name.replace(/[^a-z0-9\-_]+/gi, '_')}_repertoire_${dateStr}.pdf"`,
