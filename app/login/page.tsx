    ...
    try {
      if (FIREBASE_ENABLED && clientAuth) {
        try {
          ...
          if (!r.ok) {
            // If server can't create a Firebase session (e.g., admin not configured),
            // fall back to legacy local login using our JSON DB.
            const lr = await fetch('/api/auth/login', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ email, password }),
            });
            if (!lr.ok) throw new Error('Failed to start session');
          }
          ...
          if (!lr.ok) throw new Error('Invalid credentials');
          window.location.href = '/projects';
          return;
        }
      } else {
